...
public class ClientApp extends MessageCracker implements Application {
    ...
    public void onMessage(ExecutionReport report, SessionID sessionID) throws FieldNotFound {
        ...
        if ("4".equals(ordStatus)) {
            remaining = 0;
            canceled = origQty - filled;
        }
        ...
        if ("4".equals(ordStatus) && report.isSetField(OrigClOrdID.FIELD)) {
            clOrdId = report.getOrigClOrdID().getValue();
        }
        ...
        data.setCanceledQuantity(canceled);
        data.setSessionID(sessionID);

        final ReceivedData finalData = data;
        Platform.runLater(() -> controller.addValue(finalData));
    }
